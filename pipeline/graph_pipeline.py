...
def construct_graphs(data_frame: pd.DataFrame):
    ...
    if "smiles" not in data_frame.columns or "label" not in data_frame.columns:
        logging.error(
            f"did not find smiles or label entry in data: {data_frame.columns}"
        )
...
def split_data(data_frame: pd.DataFrame, train_split_size=0.8):
    ...
    train_split, validation_split = scaffold_split(
        data_frame, train_frac=train_split_size
    )

    return train_split, validation_split


def get_train_test_loaders(batch_size=64, train_frac=0.5):
